# Git Level 3, Task 1: Targeted Code Integration with `git cherry-pick`
...
## Table of Contents
- [Git Level 3, Task 1: Targeted Code Integration with `git cherry-pick`](#git-level-3-task-1-targeted-code-integration-with-git-cherry-pick)
...
1.  Work within the `/usr/src/kodekloudrepos/media` repository on the **Storage Server**.
...
- I ran `git log --oneline` again, this time on the master branch.

- The output showed a new commit at the very top with the message "`Update info.txt`". This new commit had a different hash than the original on the feature branch, but it contained the exact same code changes. This was the definitive proof of success.
...
<a name="deep-dive-the-power-of-git-cherry-pick-vs-git-merge"></a> This task perfectly illustrated the difference between these two powerful Git commands.

1. **`git merge feature (The Sledgehammer):`**
...
2. **`git cherry-pick <hash> (The Scalpel):`**
