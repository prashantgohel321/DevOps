# Git Level 2, Task 5: Safely Undoing Changes with `git revert`

Today's task was about a critical real-world scenario that every developer and DevOps engineer faces: a bad commit has been pushed to a shared repository, and we need to undo it. This was a fantastic lesson in how to handle mistakes safely without deleting or rewriting the project's history.
...
## Table of Contents
- [Git Level 2, Task 5: Safely Undoing Changes with `git revert`](#git-level-2-task-5-safely-undoing-changes-with-git-revert)
...
2.  Revert the latest commit (referred to as `HEAD`) to the state of the previous commit (which was the "initial commit").
3.  The new "revert" commit must have the exact commit message: `revert cluster`.
...
2.  I used `git log` to view the commit history. This was a crucial first step to understand the current state of the repository.
    ...
    ```
    The output showed the most recent commit at the top (which is `HEAD`), and the "initial commit" right below it. This confirmed the state.
...
1.  **Perform the revert without committing:** I used the `--no-commit` flag. This flag tells Git to undo the changes in the files and stage them, but to wait for me to create the commit manually. `HEAD` is a pointer that always refers to the latest commit on the current branch.
    ...
    ```
2.  **Create the custom commit:** With the changes staged, I could now create a new commit with my custom, all-lowercase message.
    ```bash
    sudo git commit -m "revert cluster"
    ```
...
1.  I ran `git log` again.
    ...
    sudo git log --oneline
    ```
2.  The output now showed my new **`revert cluster`** commit at the very top of the history. This was the definitive proof that I had successfully and safely undone the previous commit.
    ```
    a1b2c3d (HEAD -> master) revert cluster
    f9e8d7c latest commit
...
-   **Using `git reset` on a Shared Branch:** This is the most dangerous and common mistake. It would have achieved the desired code state locally but would have failed the "push your changes" part of the task (as a force push would be required) and is terrible practice.
...
-   `sudo git commit -m "revert cluster"`: The standard command to create a commit with a specific **m**essage. I used this to finalize the revert operation with the custom message required by the task.
