...
### The Task
<a name="the-task"></a>
My objective was to securely configure my Jenkins server for a new developer, `yousuf`. The specific requirements were:
1.  Install the **Matrix Authorization Strategy Plugin**.
2.  Create a new user named `yousuf` with a specific password and full name.
3.  Switch the server's security to use the **Project-based Matrix Authorization Strategy**.
4.  Configure **Global Permissions**:
    -   `yousuf` should only have `Overall/Read`.
...
5.  Configure **Project Permissions** for the existing job:
    -   `yousuf` should only have `Job/Read` permission.
...
1.  **Configure Global Security:** I went to `Manage Jenkins` > `Security`.
    ...
    -   In the permissions grid that appeared, I added both `admin` and `yousuf`.
    -   For the `admin` user, I checked the master checkbox for `Overall/Administer` to ensure I didn't lock myself out.
    -   For the `yousuf` user, I checked **only** the box for `Overall/Read`.
...
2.  **Configure Project Security:** I went back to the main dashboard and clicked on the existing job.
    ...
    -   In the new permissions grid for the job, I added the user `yousuf`.
    -   I checked the box for `Job/Read` for `yousuf` and nothing else.
    -   I clicked **Save**.

My final verification was to log out as `admin` and log in as `yousuf` to confirm that the permissions were working exactly as intended. I could see the job but couldn't configure or build it, which was the proof of success.
...
-   **Project-based vs. Global Security**: This is a key concept.
    -   **Global Security (`Manage Jenkins` > `Security`):** This is the baseline security for the entire server. The `Overall/Read` permission I gave `yousuf` allows him to log in and see the dashboard, but that's it.
    -   **Project-based Security (Inside a Job's Configuration):** This allows me to get more specific. Even though `yousuf` only has read access globally, I can grant him extra permissions on a specific job. In this case, I gave him `Job/Read`, which allows him to see the details of that one job. This is how you can give different teams access to only their own projects.
...
-   **Why is this important?**
    ...
    -   **Enhances Security:** If a user's account is compromised, the damage an attacker can do is limited by that user's permissions. If `yousuf`'s account was stolen, the attacker could only *look* at one job; they couldn't run malicious code, steal secrets (credentials), or damage the Jenkins server.
-   **How I Implemented It:**
    ...
    2.  **Established a Low Baseline:** I gave the `yousuf` user the lowest possible useful permission globally: `Overall/Read`.
