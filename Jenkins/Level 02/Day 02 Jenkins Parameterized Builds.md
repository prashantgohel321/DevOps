# Jenkins Level 2, Task 2: Creating a Parameterized Build
...
6.  Run the job at least once, selecting `Production` for the `env` parameter.
...
2.  This took me to a build screen where I could input my parameters. As required, I selected **`Production`** from the `env` dropdown menu.
...
4.  I then clicked on the new build number in the "Build History" and went to its **"Console Output"**. The log showed the correct output, proving my success:
    ...
    + echo 'The selected environment is: Production'
...
[Image of Jenkins parameterized build UI]
...
-   **The Magic of `$VARIABLE`:** The shell automatically substitutes variables that start with a `$` with their value from the environment.
    ...
    -   It looked in its environment, found a variable named `env` that Jenkins had set to `Production` (the value I selected).
    -   It then substituted the variable, so the final command that was actually executed was `echo "The selected environment is: Production"`.
