# Jenkins Level 2, Task 5: Automating a Multi-Server Log Collection Job
...
1.  Connect to **App Server 3** and copy its Apache logs (`access_log` and `error_log`).
2.  The logs needed to be transferred to the **Storage Server** into the `/usr/src/sysops` directory.
3.  The entire job had to be scheduled to run every 12 minutes (`*/12 * * * *`).
...
2.  **Add Credentials:** I went to `Manage Jenkins` > `Credentials` and added two `Username with password` credentials:
    -   One for App Server 3 (`banner`/`BigGr33n`), with ID `app-server-3-creds`.
    -   One for the Storage Server (`natasha`/`Bl@kW`), with ID `storage-server-creds`.
3.  **Configure Global SSH Sites:** I went to `Manage Jenkins` > `System` and, in the "SSH remote hosts" section, added both `stapp03` and `ststor01`, linking them to their respective credentials and using the "Test Connection" button for each to confirm success.

#### Phase 2: The Jenkins Job Configuration
1.  I created a `Freestyle project` named `copy-logs`.
2.  In the **"Build Triggers"** section, I checked "Build periodically" and entered the schedule: `*/12 * * * *`.
3.  **This was the core of my solution:** In the **"Build Steps"** section, I added a single **"Execute shell script on remote host using ssh"** step.
    -   For the **SSH Site**, I selected the App Server: `banner@stapp03:22`.
    -   In the **Command** box, I wrote a script that would run entirely on App Server 3. This script first finds the log files and then *pushes* them to the storage server using `sshpass`.
        ```bash
        ...
        sshpass -p 'Bl@kW' scp -o StrictHostKeyChecking=no /var/log/httpd/* natasha@ststor01:/usr/src/sysops
        ```
...
-   **Permissions on Destination:** The `natasha` user needed write permissions on the `/usr/src/sysops` directory on the storage server. If they didn't have it, the `scp` command would have failed.
...
-   **`[Job Name]` > `Configure`**:
    -   **`Build Triggers` > `Build periodically`**: Where I set the cron schedule (`*/12 * * * *`).
