# Git Level 2, Task 3: The Complete Git Feature Branch Workflow
...
## Table of Contents
- [Git Level 2, Task 3: The Complete Git Feature Branch Workflow](#git-level-2-task-3-the-complete-git-feature-branch-workflow)
...
2.  Create a new branch named `xfusion` from the `master` branch.
3.  Copy a file, `/tmp/index.html`, into the repository.
4.  Add and commit this new file specifically on the `xfusion` branch.
5.  Merge the `xfusion` branch back into the `master` branch.
6.  Push both the `master` and `xfusion` branches to the remote server (`origin`).
...
2.  I used the `git checkout -b` shortcut to create the new `xfusion` branch and immediately switch to it.
    ```bash
    sudo git checkout -b xfusion
    ```
3.  While on the `xfusion` branch, I performed the "development work": copying the new file, adding it to staging, and committing it.
    ```bash
    ...
    # Commit the staged file to the 'xfusion' branch history
...
1.  First, I had to switch back to the `master` branch. This is a critical step before merging.
    ...
    ```
2.  Next, I merged the `xfusion` branch into my current branch (`master`).
    ```bash
    sudo git merge xfusion
    ```
    The output showed a "fast-forward" merge, indicating the changes were applied cleanly.

3.  Finally, I pushed all my local work (the new commit on `master` and the new `xfusion` branch itself) to the central remote server.
    ```bash
...
-   **`git merge`**: This command takes the independent lines of history from two branches and integrates them into a single branch. In my case, I was on `master` and ran `git merge xfusion`, which took all the commits from the `xfusion` branch and applied them to `master`.
...
2.  **Create a New Path (`checkout -b xfusion`)**: I created a separate, isolated path to do my work without affecting the main path.
...
4.  **Re-join the Main Path (`checkout master`, `merge xfusion`)**: Once my work was complete, I went back to the main `master` path and pulled in all the changes from my feature path, combining them.
5.  **Publish Everything (`push origin ...`)**: I updated the central server with both the new state of the `master` branch and the history of the `xfusion` branch for my teammates to see.
...
-   **Forgetting to Push Both Branches:** If I had only pushed `master` (`git push origin master`), my new feature would be in the main codebase, but the `xfusion` branch itself would only exist on my local machine. Pushing both shares the complete history of the work.
...
-   `sudo git checkout -b xfusion`: **C**hecks **out** a **n**ew **b**ranch named `xfusion` and switches to it.
...
-   `sudo git merge xfusion`: Merges all commits from the `xfusion` branch into the current branch (`master`).
-   `sudo git push origin master xfusion`: Pushes the state of the local `master` and `xfusion` branches to the remote server named `origin`.
