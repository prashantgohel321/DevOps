# Git Level 2, Task 2: Managing Development with Git Branches
...
I learned how to create a new branch from the existing `master` branch. The task also reinforced the importance of understanding the underlying Linux file permissions, as I needed to use `sudo` to perform the Git operation in a system-owned directory. This document is my detailed, first-person guide to that process.
...
1.  Navigate to the `/usr/src/kodekloudrepos/media` Git repository.
2.  Create a new branch named `xfusioncorp_media` from the `master` branch.
...
3.  **Handle Permissions:** I knew that the repository was in a system directory (`/usr/src`) and likely owned by `root`. Therefore, to create a branch (which writes to the `.git` directory), I would need `sudo`.

4.  **Create the Branch:** I used the `git branch` command, which is designed to simply create a new branch without switching to it.
    ```bash
    sudo git branch xfusioncorp_media
    ```

5.  **Verification:** The final and most important step was to confirm that the new branch was created. I used the `git branch` command without any arguments to list all local branches.
    ```bash
    sudo git branch
    ```
...
-   **Git Branch:** A branch is an independent line of development. I like to think of it as a movable pointer to a specific commit. When I created my new branch, I was essentially creating a new pointer named `xfusioncorp_media` that started at the exact same point as the `master` branch.
...
-   **Permissions:** Forgetting `sudo` would cause a `Permission denied` error because the repo is owned by `root`.
-   **Wrong Directory:** Running the `git branch` command outside of the repository directory would result in a `fatal: not a git repository` error.
-   **Using `checkout -b`:** This would have been an incorrect solution for this specific task, as the goal was only to *create* the branch, not to switch to it.
...
-   `cd /usr/src/kodekloudrepos/media`: The **c**hange **d**irectory command, used to navigate into the correct Git repository.
...
-   `sudo git branch xfusioncorp_media`: The main command for this task. When used with an argument, it creates a new branch with the specified name. It branches off from the current HEAD commit (the commit I was on in `master`).
